...
def change_entropy_rider(envt, action, driver_num):
    ...
    for i in range(len(envt.requests_region)):
        if (envt.requests_region[i] + new_requests[i]) == 0:
            ...
            percent_success_new.append(
                (envt.success_region[i] + new_requests[i]) / (envt.requests_region[i] + new_requests[i]))
...
def get_entropy_list(l):
    ...
    N = len(l)
    l[l == 0] = 1e-8
    # alternate log trick implementation
    # return -1/(N) * (np.sum(np.log(l)) - N*np.log(ybar)).
    if ybar != 0:
